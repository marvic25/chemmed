...
/* MAIN */
.main{display:grid;grid-template-columns:1fr;gap:9px;padding:9px;overflow:hidden;transition:grid-template-columns .3s ease}
.main.pnl-open{grid-template-columns:388px 1fr}
.main-btn-panel{position:fixed;bottom:30px;left:14px;background:var(--ac);color:#fff;border:none;border-radius:50%;width:48px;height:48px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:1.1rem;box-shadow:0 4px 16px rgba(79,95,212,.35);transition:all .2s;z-index:100}
.main-btn-panel:hover{transform:scale(1.1);box-shadow:0 6px 20px rgba(79,95,212,.45)}
.main-btn-panel.hidden{opacity:0;pointer-events:none}
.pnl-close-btn{position:absolute;top:5px;right:5px;background:transparent;border:none;color:var(--tx);font-size:1.2rem;cursor:pointer;opacity:.6;transition:opacity .2s;z-index:50}
.pnl-close-btn:hover{opacity:1}
/* PANEL */
...
@media(max-width:768px){
  ...
  .app{grid-template-rows:45px 1fr 20px}
  .hdr{padding:0 12px;gap:6px}
  .hl{font-size:.9rem}
  .hs{font-size:.55rem}
  .hpills{gap:4px}
  .hpill{padding:2px 6px;font-size:.55rem}
  .main{padding:6px;gap:6px}
  .main.pnl-open{grid-template-columns:1fr}
  .pnl{border-radius:8px}
  .tb{padding:4px 5px;gap:2px;overflow-x:auto}
  .btn{height:24px;padding:0 6px;font-size:.6rem}
  .btn[data-tip]:hover::after{max-width:120px;font-size:.55rem}
  .tbl{font-size:.5rem}
  .elb{width:24px;height:24px;font-size:.65rem}
  .asel{height:24px;font-size:.6rem}
  #mol-cv{min-height:200px}
  #svc{height:200px}
  .tabs{flex-wrap:wrap;gap:4px}
  .tab{font-size:.7rem;padding:6px 8px}
  .tcon{padding:8px}
  .sec{font-size:.55rem}
  ...
  .pv{font-size:1rem}
  .pmd{font-size:.65rem}
  .pxmpl{font-size:.6rem;max-height:60px}
  .main-btn-panel{bottom:20px;left:10px;width:44px;height:44px;font-size:.9rem}
  .pnl-close-btn{font-size:1rem;top:3px;right:3px}
}
@media(max-width:480px){
  html,body{font-size:11px}
  .app{grid-template-rows:40px 1fr 18px}
  .hdr{padding:0 8px;gap:4px}
  .hl{font-size:.8rem}
  .hpill{padding:1px 4px;font-size:.5rem}
  .main{padding:4px;gap:4px}
  .tb{padding:3px 4px;gap:1px}
  .btn{height:22px;padding:0 4px;font-size:.55rem}
  .elb{width:22px;height:22px;font-size:.6rem}
  #mol-cv{min-height:180px}
  #svc{height:150px}
  .tabs{gap:2px}
  .tab{font-size:.65rem;padding:4px 6px}
  .tcon{padding:6px}
  .main-btn-panel{width:40px;height:40px;font-size:.8rem;bottom:16px;left:8px}
}
</style>
...
<div class="app">
  ...
  <div class="main">
    ...
    <div class="pnl">
      <div class="tb">
        ...
        <button class="btn" onclick="editor.fitToView()" data-tip="Ajustar canvas ao tamanho do desenho">◀▶</button>
        ...
        <button class="btn" id="lone-btn" onclick="toggleLonePairs()" data-tip="Mostrar/ocultar pares de elétrons livres">🔴 Pares</button>
    ...
    <!-- ANALYSIS -->
    <div class="pnl" id="analysis-pnl">
      <button class="pnl-close-btn" onclick="toggleAnalysisPanel()" title="Fechar análise">✕</button>
      <div class="tabs">
  ...
  <button class="main-btn-panel" id="main-toggle-btn" onclick="toggleAnalysisPanel()" title="Abrir/fechar análise">📊</button>

  <!-- FOOTER -->
  <footer class="ftr">
    <span>⚗️ ChemMed · Desenvolvido por <strong>Msc. Marcos Gregório</strong></span>
    <span>·</span>
    <span>UNINASSAU — Química Medicinal</span>
    <span>·</span>
...
function toggleAnalysisPanel(){
  ...
  main.classList.toggle('pnl-open');
  btn.classList.toggle('hidden',main.classList.contains('pnl-open'));
}
...
function validateBondCreation(editor,fromAtomId,toAtomId,order,excludeBondId=null){
  ...
  const sumOther=(atomId)=>(editor.adj[atomId]||[]).reduce((sum,nb)=>{
    if(nb.bid===excludeBondId)return sum;
    const b=editor.bonds.find(b=>b.id===nb.bid);
    return sum+(b?.order||1);
  },0);
  const fBonds=sumOther(fromAtomId),tBonds=sumOther(toAtomId);
...
function renderLonePairs(cx,x,y,atomId,editor){
  if(!editor.showLonePairs)return;
  const a=editor.atoms.find(a=>a.id===atomId);
  if(!a)return;
  const v=getValence(a.el);
...
class MolEditor{
  ...
  _mu(e){
    ...
    if(this.drag.t==='bd'){
      ...
      if(tgt>=0&&tgt!==from){
        ...
        if(ex){
          const newOrder=ex.order===3?1:ex.order+1;
          const check=validateBondCreation(this,from,tgt,newOrder,ex.id);
          if(check.valid){ex.order=newOrder;changed=true;}
          else setEditorStatus(check.msg);
        }else{
          const check=validateBondCreation(this,from,tgt,this.bo);
          if(check.valid){this._addB(from,tgt,this.bo);changed=true;}
          else setEditorStatus(check.msg);
        }
      }else if(tgt<0){
        const fa=this.atoms.find(a=>a.id===from);
        if(fa&&Math.hypot(x-fa.x,y-fa.y)>22){
          const newElMax=getValence(this.el).maxBonds;
          const fromBonds=(this.adj[from]||[]).reduce((sum,nb)=>sum+(this.bonds.find(b=>b.id===nb.bid)?.order||1),0);
          if(this.bo>newElMax){
            setEditorStatus(`${this.el} excede valência (máx ${newElMax})`);
          }else if(fromBonds+this.bo>getValence(fa.el).maxBonds){
            setEditorStatus(`${fa.el} excede valência (máx ${getValence(fa.el).maxBonds})`);
          }else{
  ...
  fitToView(){
    ...
    const padding=60;
    const maxW=this._w-padding,maxH=this._h-padding;
    const scaleX=w>0?maxW/w:1,scaleY=h>0?maxH/h:1;
    const scale=Math.min(scaleX,scaleY,1.5);
    const cx=(minX+maxX)/2,cy=(minY+maxY)/2;
    this.atoms.forEach(a=>{
      a.x=(a.x-cx)*scale+this._w/2;
      a.y=(a.y-cy)*scale+this._h/2;
    });
    this._saveHist();setEditorStatus('Canvas ajustado ao tamanho do desenho');this.draw();
  }
  ...
  draw(){
    ...
    if(this.drag&&this.drag.t==='bd'){
      ...
      if(fa){
        ...
        const isSnapTarget=tgt>=0&&tgt!==fa.id;
        const endX=isSnapTarget?this.atoms.find(a=>a.id===tgt).x:tx;
        const endY=isSnapTarget?this.atoms.find(a=>a.id===tgt).y:ty;

        cx.save();
        // bond line
        cx.strokeStyle=isSnapTarget?this._cv('--ok'):this._cv('--ac');
        cx.lineWidth=2.5;cx.setLineDash([4,4]);cx.globalAlpha=.7;
        cx.beginPath();cx.moveTo(fa.x,fa.y);cx.lineTo(endX,endY);cx.stroke();

        // bond order indicator
        if(this.bo>1){
          cx.fillStyle=this._cv('--ac');cx.font='bold 8px monospace';
          cx.textAlign='center';cx.textBaseline='middle';
          cx.globalAlpha=.5;cx.fillText(this.bo,fa.x+(endX-fa.x)/2,fa.y+(endY-fa.y)/2-6);
        }

        // new atom preview (H or other element)
        if(tgt<0&&ex){
          cx.globalAlpha=this.el==='H'?.5:.4;
          cx.fillStyle=this.el==='H'?this._cv('--pc'):this._cv('--ac');
          cx.beginPath();cx.arc(tx,ty,ATOM_R[this.el]||12,0,2*Math.PI);cx.fill();
          cx.globalAlpha=.85;cx.font=`bold 11px 'Space Mono',monospace`;cx.textAlign='center';cx.textBaseline='middle';
          cx.fillStyle='#fff';cx.fillText(this.el,tx,ty);
        }

        // snap target highlight
        if(isSnapTarget){
          const ta=this.atoms.find(a=>a.id===tgt);
          const tr=ATOM_R[ta.el]||12;
          cx.strokeStyle=this._cv('--ok');cx.lineWidth=2;cx.globalAlpha=.6;
          cx.beginPath();cx.arc(ta.x,ta.y,tr+5,0,2*Math.PI);cx.stroke();
        }
  ...
  toSMILES(){
    ...
    const dfs=(id,pb)=>{
      ...
      for(const nb of(this.adj[id]||[])){
        ...
        if(rbs.has(nb.bid)){const num=rbs.get(nb.bid);const b=this.bonds.find(b=>b.id===nb.bid);s+=(b.order===2?'=':b.order===3?'#':'')+(num>9?`%${num}`:String(num));}
      }
      const ch=(this.adj[id]||[]).filter(nb=>nb.bid!==pb&&!rbs.has(nb.bid));
...
function toggleLonePairs(){
  editor.showLonePairs=!editor.showLonePairs;
  document.getElementById('lone-btn').classList.toggle('act',editor.showLonePairs);
  setEditorStatus(editor.showLonePairs?'Pares de elétrons livres visíveis':'Pares de elétrons ocultados');
  editor.draw();
}
